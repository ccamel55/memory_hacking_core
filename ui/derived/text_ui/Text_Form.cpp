...
void Text_Form::render() {
	...
	if (!(getFlags() & E_UI_FLAGS::UI_INPUT_ONLY)) {
		...
		UI_RenderFactory::get().getImpl()->drawString(position._x + UI_FORM::PINNED_OFFSET._x + UI_FORM::CONTENT_POS._x + 8, position._y + (UI_FORM::CONTENT_POS._y / 2), UI_FONTS::WINDOW_FONT, UI_COLORS::FORM_TEXT_LABEL, getName(), E_FONT_FLAGS::FONT_CENTER_Y);
		...
		UI_RenderFactory::get().getImpl()->drawString(position._x, position._y, UI_FONTS::TEXT_UI_FONT, UI_COLORS::WHITE, "[  ]", E_FONT_FLAGS::FONT_OUTLINE);
		UI_RenderFactory::get().getImpl()->drawString(position._x + 25, position._y, UI_FONTS::TEXT_UI_FONT, UI_COLORS::WHITE, getName(), E_FONT_FLAGS::FONT_OUTLINE);

		if (_expanded) {

			UI_RenderFactory::get().getImpl()->drawRectFill(position._x + 8, position._y + 5, 2, 6, UI_COLORS::GREEN);
			UI_RenderFactory::get().getImpl()->drawRect(position._x + 8, position._y + 5, 2, 6, UI_COLORS::BLACK);
		}
		else {

			UI_RenderFactory::get().getImpl()->drawRectFill(position._x + 6, position._y + 7, 6, 2, UI_COLORS::WHITE);
			UI_RenderFactory::get().getImpl()->drawRect(position._x + 6, position._y + 7, 6, 2, UI_COLORS::BLACK);
		}
	}
