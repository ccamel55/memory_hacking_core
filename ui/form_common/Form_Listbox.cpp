...
void Form_Listbox::render() {
	...
	for (size_t i = _scroll; i < std::min<size_t>(UI_CONTROL::LISTBOX_ITEMS + _scroll, _items->size()); i++) {
		UI_RENDER::drawString(_textPosition._x , _textPosition._y + ((i - _scroll) * UI_CONTROL::LISTBOX_ITEM_SPACING._y), UI_FONTS::CONTROL_FONT, (i == _selected) ? UI_COLORS::LIGHT_BLUE : (i == _cursor) ? UI_COLORS::CONTROL_FILL_3 : UI_COLORS::WHITE, _items->at(i));
	}
}
