...
void Form_Listbox::render() {
	...
	UI_RENDER::drawRectFilled(pos._x, pos._y, size._x, size._y, UI_COLORS::CONTROL_FILL);
	...
	for (size_t i = _scroll; i < std::min<size_t>(UI_CONTROL::LISTBOX_ITEMS + _scroll, _items->size()); i++) {
		UI_RENDER::drawText(_textPosition._x , _textPosition._y + ((i - _scroll) * UI_CONTROL::LISTBOX_ITEM_SPACING._y), UI_FONTS::CONTROL_FONT, (i == _selected) ? UI_COLORS::LIGHT_BLUE : (i == _cursor) ? UI_COLORS::CONTROL_FILL_3 : UI_COLORS::WHITE, _items->at(i));
	}
}
